...
async function main() {
  const user = await prisma.user.upsert({ // <-- Změněno z 'uzivatel' na 'user'
    where: { email: 'test@goshary.cz' },
    update: {},
    create: {
      name: 'Testovací Uživatel', // <-- Změněno z 'jmeno' na 'name'
  ...
  const nabidka = await prisma.inzerat.create({
    data: {
      ...
      overeno: true,
      hodnoceni: 4.8,
      autorId: user.id, // <-- Použije se id z nově vytvořeného uživatele
      fotky: {
        create: [
          {
