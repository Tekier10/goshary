...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    let updatedUser;
    if (existingFavorite) {
      // Pokud ano, ODEBEREME ho (disconnect)
      ...
      return res.status(200).json({ message: 'Odebráno z oblíbených' });
    } else {
      // Pokud ne, PŘIDÁME ho (connect)
      updatedUser = await prisma.user.update({
        where: { id: userId },
        data: {
      ...
      return res.status(200).json({ message: 'Přidáno do oblíbených' });
    }
  } catch (error) {
    console.error(error);
